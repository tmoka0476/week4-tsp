...
### 結果
`/answer/NN_2opt_oropt`以下で`./solver_tomo -i INPUT_FILE_NAME -n NUMBER_OF_POINTS -o OUTPUT_FILE_NAME`を実行してください。  
  

各手法を用いて計算した結果は以下のようになりました。
...
改善手法を加えるごとにより短い経路を探索できていることがわかります。特にNN+2-optでの改善が大きく、さらにor-optを加えるとより良い結果が得られています。
### 考えたこと
- 領域を分割して、各領域ごとにNN → 2つの領域から近い点を2つずつ選んで経路をマージする という手法もありそうだなと思いました。けれどより最適に近い解をみる限り、そんなきれいに領域ごとに分かれているような経路にはなっていないのであんまり効果はないかも……
- 並列計算を使えば高速に計算できる？
- 距離を求めるときにいちいち計算せずに、事前に計算して二次元配列に入れておいて参照するようにすれば計算時間短縮になりますね。
- 遺伝的アルゴリズムで解くやり方もあるみたいで面白い。

### 感想
- 時間が足りなくて3-opt法が実装できなかったのが残念。
- スコアボード見たらN=2048で4万台を叩き出している人がいてすごっ……となる。コードを読んで勉強させていただきます……
- ゲーム性が高くて面白かったです！ 他の人のスコアが見られることで競争心を掻き立てられました。
